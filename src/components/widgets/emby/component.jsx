...
const brandColor = {
  ...
  streamBadge: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
}

const MAX_STREAMS = 5

const EmbyWidget = ({ widget, onDelete, onRefresh }) => {
  ...
  const fetchData = async (isInitial = false, isManual = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/widgets/${widget.id}/proxy?endpoint=emby`, { credentials: 'include' })
      if (!response.ok) throw new Error('Failed to fetch Emby data')
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-8 h-8 rounded-lg bg-slate-800/80 border border-slate-700/50 flex items-center justify-center flex-shrink-0 overflow-hidden">
              {!iconError ? (
                <img src="/icons/custom/emby.webp" alt="Emby" className="w-6 h-6 object-contain" onError={() => setIconError(true)} />
              ) : (
                <Play className={cn('w-4 h-4', brandColor.text)} fill="currentColor" />
              )}
            </div>
            <span className="font-semibold text-slate-100 text-sm truncate">{widget.title || 'Emby'}</span>
                ...
                <motion.div key={idx} variants={staggerItem}>
                  <div
                    ...
                    className={cn(
                      ...
                      'hover:bg-slate-800/70 hover:border-emerald-500/25'
                    )}
                  >
      ...
      {widget.url && (
        <CardFooter className="p-0 border-t border-slate-800">
          <a href={widget.url} target="_blank" rel="noopener noreferrer" className="w-full">
            ...
            >
              Open Emby <ArrowUpRight className="w-3.5 h-3.5" />
            </Button>
          </a>
        </CardFooter>
      )}
